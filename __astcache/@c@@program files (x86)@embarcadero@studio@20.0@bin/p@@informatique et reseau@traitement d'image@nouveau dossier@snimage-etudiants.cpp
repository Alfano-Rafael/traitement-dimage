...
{}
void SNImage::Histogrammes() // nécessite histogrammeBleu.bmp histogrammeVert.bmp histogrammeRouge.bmp 256x256pixels
{}
...
{	ifstream entree;
	...
	i = hauteur - 1;
	j = 0; // i indice des lignes
	if (signature != 0x4D42)
		cout << "Vous devez choisir un bitmap .bmp";
	else if (format != 0x180001)
		cout << "Vous devez choisir un bitmap 24 bits";
	else 
...
{	Coordonnee coor;
	...
	coor.ligne = 0;
	coor.colonne = largeur - largeurDroite;
	Colorier(coor, largeurDroite, hauteur, couleur);
	coor.ligne = hauteur - largeurBas;
	coor.colonne = 0;
...
{	Coordonnee coord = {-1, -1};
	...
	for (int i = 0; i < hauteur; i++) 
	{	for (int j = 0; j < largeur; j++) 
		{	if (image[i][j].bleu == couleur.bleu 
			 && image[i][j].vert == couleur.vert
			 && image[i][j].rouge == couleur.rouge)
			{	zoneTrouve = true;
				for (int m = 0; m < h; m++) 
				{	for (int n = 0; n < l; n++) 
					{	if (image[i + m][j + n].bleu != couleur.bleu 
						 || image[i + m][j + n].vert != couleur.vert 
						 || image[i + m][j + n].rouge != couleur.rouge)
							zoneTrouve = false;
					}
				}
				if (zoneTrouve) 
...
{	Coordonnee debut;
	debut.ligne = coord.ligne - taille / 2;
	debut.colonne = coord.colonne - epaisseur / 2;
	if (debut.ligne < 0)
		debut.ligne = 0;
	if (debut.colonne < 0)
		debut.colonne = 0;
	Colorier(debut, epaisseur, taille, couleur);
	debut.ligne = coord.ligne - epaisseur / 2;
	debut.colonne = coord.colonne - taille / 2;
	if (debut.ligne < 0)
		debut.ligne = 0;
	if (debut.colonne < 0)
		debut.colonne = 0;
...
{	Coordonnee debut, fin;
	...
	case 'a':
		debut = coord;
		debut.ligne -= demiepaisseur;
		debut.colonne -= demiepaisseur;
		Colorier(debut, demitaille + epaisseur, epaisseur, couleur);
		break;
	case 'b':
		debut = coord;
		debut.ligne -= demiepaisseur;
		debut.colonne -= demiepaisseur;
		debut.colonne += demitaille;
		Colorier(debut, epaisseur, demitaille + epaisseur, couleur);
		break;
	case 'f':
		debut = coord;
		debut.ligne -= demiepaisseur;
	...
	case 'g':
		...
		debut.ligne -= demiepaisseur;
		debut.colonne -= demiepaisseur;
		debut.ligne += demitaille;
		Colorier(debut, demitaille + epaisseur, epaisseur, couleur);
		break;
	case 'c':
		debut = coord;
		debut.ligne -= demiepaisseur - demitaille;
		debut.colonne -= demiepaisseur;
		debut.colonne += demitaille;
		Colorier(debut, epaisseur, demitaille + epaisseur, couleur);
		break;
	...
	case 'h': // 10h
		debut = coord;
		fin = coord;
		debut.colonne = debut.colonne + quarttaille; 
		debut.ligne = debut.ligne + demitaille;
		fin.colonne = fin.colonne; 
		fin.ligne = fin.ligne;
	...
	case 'l': // 12h+6h
		...
		debut.ligne -= demiepaisseur;
		debut.colonne -= demiepaisseur;
		debut.colonne += quarttaille;
		Colorier(debut, epaisseur, taille + epaisseur, couleur);
		break;
	}
}
...
{	if (debut.ligne > fin.ligne) 
	...
	if (l == 0)
		Colorier(debut, 1, h, couleur);
	else 
	{	if (h == 0) 
		{	if (debut.colonne <= fin.colonne)
				Colorier(debut, l, 1, couleur);
			else 
			{	Coordonnee d;
				d.ligne = debut.ligne;
...
void SNImage::AfficherImageEnCaracteres4Seuils() 
{
	...
	{	for (int j = 0; j < largeur; j++) 
		{	if (image[i][j].bleu < 64)
				cout << 'X';
			else if (image[i][j].bleu < 128)
				cout << 'x';
			else if (image[i][j].bleu < 196)
				cout << '.';
			else
				cout << ' ';
		}
...
{	NiveauDeGris();
	...
	{	for (int j = 0; j < largeur; j++) 
		{	if (image[i][j].bleu < 25)
				cout << caracteres[0];
			else if (image[i][j].bleu < 50)
				cout << caracteres[1];
			else if (image[i][j].bleu < 75)
				cout << caracteres[2];
			else if (image[i][j].bleu < 100)
				cout << caracteres[3];
			else if (image[i][j].bleu < 125)
				cout << caracteres[4];
			else if (image[i][j].bleu < 150)
				cout << caracteres[5];
			else if (image[i][j].bleu < 175)
				cout << caracteres[6];
			else if (image[i][j].bleu < 200)
				cout << caracteres[7];
			else if (image[i][j].bleu < 225)
				cout << caracteres[8];
			else
				cout << caracteres[9];
		}
...
{	Pixel **temp;
	...
	for (int i = 0; i < hauteur; i++)
		for (int j = 0; j < largeur; j++)
			temp[i][j] = image[i][j]; // sauvegarde image de départ
	for (int s = 0; s < 256; s++) 
	{	nb = 0;
		SeuillerNoirBlanc(s);
		for (int i = 0; i < hauteur; i++)
		...
		if (!seuilOK)
			for (int i = 0; i < hauteur; i++)
				for (int j = 0; j < largeur; j++)
					image[i][j] = temp[i][j]; // annulation traitement
	}
	for (int i = 0; i < dimensionMax; i++)
		delete[]temp[i];
	delete[]temp;
...
{	char texte[50];
	...
	int decalage = 0;
	if (!droite)
		decalage = -tailleTexte * strlen(texte);
...
{	taille = Arrondi((float)taille / 2.0);
	...
	coord2.colonne += Arrondi((float)taille / 2.0); 
	coord3.ligne += taille;
	if (taille % 2)
		coord3.ligne++;
	coord4.ligne += taille;
	if (taille % 2)
		coord4.ligne++;
	coord4.colonne += Arrondi((float)taille / 2.0); 
	char segments[4][45] = {"", "", "", ""};
	bool espace;
	for (int i = 0; i < message.length(); i++) 
	{	espace = false;
		...
		{
		case '¤':
			strcpy(segments[0], "abcdefghijk");
			strcpy(segments[1], "abcdefghijk");
			strcpy(segments[2], "abcdefghijk");
			strcpy(segments[3], "abcdefghijk");
			break;
		case '@':
			strcpy(segments[0], "aefbc");
			strcpy(segments[1], "abcd");
			strcpy(segments[2], "efd");
			strcpy(segments[3], "dc");
			break;
		case '0':
			strcpy(segments[0], "aef");
			strcpy(segments[1], "abcij");
			strcpy(segments[2], "defij");
			strcpy(segments[3], "bcd");
			break;
		case '1':
			strcpy(segments[0], "ijbc");
			strcpy(segments[1], "");
			strcpy(segments[2], "bc");
			strcpy(segments[3], "");
			break;
		case '2':
			strcpy(segments[0], "ad");
			strcpy(segments[1], "abcd");
			strcpy(segments[2], "efd");
			strcpy(segments[3], "d");
			break;
		case '3':
			strcpy(segments[0], "a");
			strcpy(segments[1], "abc");
			strcpy(segments[2], "ad");
			strcpy(segments[3], "abcd");
			break;
		case '4':
			strcpy(segments[0], "fed");
			strcpy(segments[1], "bcd");
			strcpy(segments[2], "");
			strcpy(segments[3], "bc");
			break;
		case '5':
			strcpy(segments[0], "aefd");
			strcpy(segments[1], "ad");
			strcpy(segments[2], "d");
			strcpy(segments[3], "dbc");
			break;
		case '6':
			strcpy(segments[0], "aefd");
			strcpy(segments[1], "ad");
			strcpy(segments[2], "def");
			strcpy(segments[3], "dbc");
			break;
		case '7':
			strcpy(segments[0], "a");
		...
		case ']':
			...
			break;
		case '(':
			strcpy(segments[0], "ij");
			strcpy(segments[1], "");
			strcpy(segments[2], "hk");
			strcpy(segments[3], "");
			break;
		case ')':
			strcpy(segments[0], "");
			strcpy(segments[1], "hk");
			strcpy(segments[2], "");
			strcpy(segments[3], "ij");
			break;
		case ':':
			strcpy(segments[0], "b");
			strcpy(segments[1], "");
		...
		case '=':
			...
			break;
		case '\\':
			strcpy(segments[0], "hk");
			strcpy(segments[1], "");
			strcpy(segments[2], "");
			strcpy(segments[3], "hk");
			break;
		case '/':
			strcpy(segments[0], "");
			strcpy(segments[1], "ij");
			strcpy(segments[2], "ij");
			strcpy(segments[3], "");
			break;
		case '\'':
			strcpy(segments[0], "");
		...
		case 'A':
			strcpy(segments[0], "aefd");
			strcpy(segments[1], "abcd");
			strcpy(segments[2], "ef");
			strcpy(segments[3], "bc");
			break;
		case 'B':
			strcpy(segments[0], "afed");
			strcpy(segments[1], "hjd");
			strcpy(segments[2], "efd");
			strcpy(segments[3], "bcd");
			break;
		case 'C':
		...
		case 'H':
			...
			break;
		case 'I':
			strcpy(segments[0], "abc");
			strcpy(segments[1], "a");
			strcpy(segments[2], "dbc");
			strcpy(segments[3], "d");
			break;
		case 'J':
			strcpy(segments[0], "abc");
			strcpy(segments[1], "a");
			strcpy(segments[2], "edbc");
			strcpy(segments[3], "");
			break;
		case 'K':
			strcpy(segments[0], "fed");
			strcpy(segments[1], "ji");
			strcpy(segments[2], "fe");
			strcpy(segments[3], "hk");
			break;
		case 'L':
			strcpy(segments[0], "fe");
			strcpy(segments[1], "");
			strcpy(segments[2], "efd");
			strcpy(segments[3], "d");
			break;
		case 'M':
			strcpy(segments[0], "fehk");
			strcpy(segments[1], "ijbc");
			strcpy(segments[2], "ef");
			strcpy(segments[3], "bc");
			break;
		case 'N':
			strcpy(segments[0], "hkfe");
			strcpy(segments[1], "bc");
			strcpy(segments[2], "ef");
			strcpy(segments[3], "hkbc");
			break;
		case 'O':
			strcpy(segments[0], "afe");
			strcpy(segments[1], "abc");
		...
		case 'W':
			strcpy(segments[0], "fe");
			strcpy(segments[1], "bc");
			strcpy(segments[2], "efij");
			strcpy(segments[3], "bchk");
			break;
		case 'X':
			strcpy(segments[0], "hk");
			strcpy(segments[1], "ij");
		...
		case 'Z':
			strcpy(segments[0], "a");
			strcpy(segments[1], "aij");
			strcpy(segments[2], "ijd");
			strcpy(segments[3], "d");
			break;
		case 'à':
			strcpy(segments[0], "d");
			strcpy(segments[1], "dh");
			strcpy(segments[2], "edg");
			strcpy(segments[3], "gdbc");
			break;
		case 'a':
		case 'â':
			strcpy(segments[0], "d");
		...
		case 'd':
			strcpy(segments[0], "d");
			strcpy(segments[1], "dbc");
			strcpy(segments[2], "efd");
			strcpy(segments[3], "bcd");
			break;
		case 'é':
			strcpy(segments[0], "di");
			strcpy(segments[1], "d");
			strcpy(segments[2], "efdg");
			strcpy(segments[3], "dbg");
			break;
		case 'è':
			strcpy(segments[0], "d");
			strcpy(segments[1], "dh");
		...
		case 'ë':
			...
			break;
		case 'f':
			strcpy(segments[0], "ged");
			strcpy(segments[1], "g");
			strcpy(segments[2], "ef");
			strcpy(segments[3], "");
			break;
		case 'g':
			strcpy(segments[0], "egd");
			strcpy(segments[1], "gcd");
			strcpy(segments[2], "ed");
			strcpy(segments[3], "bcd");
			break;
		case 'h':
			strcpy(segments[0], "fed");
		...
		case 'p':
			strcpy(segments[0], "d");
			strcpy(segments[1], "d");
			strcpy(segments[2], "efg");
			strcpy(segments[3], "gb");
			break;
		case 'q':
			strcpy(segments[0], "d");
			strcpy(segments[1], "d");
			strcpy(segments[2], "fg");
			strcpy(segments[3], "bcg");
			break;
		case 'r':
			strcpy(segments[0], "d");
			strcpy(segments[1], "d");
		...
		case 's':
			strcpy(segments[0], "d");
			strcpy(segments[1], "d");
			strcpy(segments[2], "fgd");
			strcpy(segments[3], "gcd");
			break;
		case 't':
			strcpy(segments[0], "ed");
			strcpy(segments[1], "");
			strcpy(segments[2], "efd");
			strcpy(segments[3], "d");
			break;
		...
		case 'x':
			strcpy(segments[0], "");
			strcpy(segments[1], "");
			strcpy(segments[2], "hkij");
			strcpy(segments[3], "");
			break;
		case 'y':
			strcpy(segments[0], "");
			strcpy(segments[1], "");
			strcpy(segments[2], "hij");
			strcpy(segments[3], "");
			break;
		case 'z':
			strcpy(segments[0], "d");
		...
		{	for (int n = 0; n < strlen(segments[0]); n++)
				ChangerCouleurSegment(coord, taille, epaisseur, couleur,
				segments[0][n]);
			for (int n = 0; n < strlen(segments[1]); n++)
				ChangerCouleurSegment(coord2, taille, epaisseur, couleur,
				segments[1][n]);
			for (int n = 0; n < strlen(segments[2]); n++)
				ChangerCouleurSegment(coord3, taille, epaisseur, couleur,
				segments[2][n]);
			for (int n = 0; n < strlen(segments[3]); n++)
				ChangerCouleurSegment(coord4, taille, epaisseur, couleur,
				segments[3][n]);
		}
